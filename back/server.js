...
app.post("/api/login", (req, res) => {
  ...
  if (!login || !password) {
    return res
      .status(400)
      .json({ success: false, message: "Login et mot de passe requis" });
  }
...
app.post("/api/inscription", (req, res) => {
  ...
  db.query(checkQuery, [username, email], (err, results) => {
    ...
    if (results.length > 0)
      return res
        .status(409)
        .json({ success: false, message: "Utilisateur ou email déjà utilisé" });
  ...
  try {
    ...
    setInterval(async () => {
      try {
        ...
        console.error(
          "Erreur de communication avec le Poseidon :",
          errLoop.message,
        );
...
app.post("/api/relais/:numRelais", authMiddleware, async (req, res) => {
  ...
  if (![1, 2, 3, 4].includes(num)) {
    return res
      .status(400)
      .json({ success: false, message: "Relais invalide (1 à 4)" });
  }
...
async function regulateLoop() {
  ...
  socket.on("connect", async () => {
    try {
      ...
      db.query(
        "SELECT temperature, humidite_moyenne, humidite_air, relai_0, relai_1, relai_2, relai_3 FROM Consigne LIMIT 1",
        async (err, rows) => {
  ...
  db.query(query, (err, results) => {
    ...
    if (results.length === 0) {
      return res.json({
        success: true,
        controles: null,
        message: "Aucun contrôle enregistré",
      });
    }
...
app.post("/api/controles", authMiddleware, isAdminMiddleware, (req, res) => {
  ...
  if (!irrigation || !misting || !ventilation || !heating) {
    return res
      .status(400)
      .json({ success: false, message: "Paramètres incomplets" });
  }
  ...
  if (ventilation.mode === "active" && heating.mode === "active") {
    return res.status(400).json({
      success: false,
      message:
        "Le chauffage ne peut pas être actif si la ventilation est active",
    });
  }
...
app.get("/api/admin/users", authMiddleware, isAdminMiddleware, (req, res) => {
  const query =
    "SELECT id, login, mail, role FROM Utilisateur ORDER BY login ASC";
